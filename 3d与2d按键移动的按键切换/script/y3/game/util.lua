...
function y3.move_along_path(path, patrol_mode, is_attack, nearby_start_point, is_deviate_return)
    return game_api.create_unit_command_move_along_road(path.base(), patrol_mode, is_attack, nearby_start_point.base(), is_deviate_return)
end
...
function y3.cast_ability(ability, point1, point2,  unit, item, dest)
    return game_api.create_unit_command_use_skill(ability.base(), point1.base(), point2.base(), unit.base(), item.base(), dest.base())
end
...
---使用物品
function y3.use_item(item, point1, point2, point3, unit, target_item, dest)
    return game_api.create_unit_command_use_item(item.base(), point1.base(), point2.base(), point3.base(), unit.base(), target_item.base(), dest.base())
end
...
---为玩家切换画风
function y3.set_post_effect(player, processing)
    game_api.set_post_effect(player.base(), processing)
end
...
function y3.start_new_round_of_game(is_quick_reset)
    game_api.request_new_round(is_quick_reset)
end
...
function y3.get_random_pool_probability(pool, value)
    return game_api.get_random_pool_probability(pool, value)
end
