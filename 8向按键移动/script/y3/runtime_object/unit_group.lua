...
function unit_group.filter_unit_in_area(point, shape, owner_player, visible_player, invisible_player, not_in_unit_group, has_tag,
    ...
    local py_unit_group = y3.filter_unit_id_list_in_area_v2(point.base(), shape, owner_player.base(), visible_player.base(), invisible_player.base(), not_in_unit_group.base(), has_tag, not_has_tag,
            unit_name, not_unit, unit_type, in_state, not_in_state, is_including_dead, max_count, filtering_rules)
    return unit_group.create_lua_unit_group_from_py(py_unit_group)
end
